...
case "$7" in
	...
	sz)
		...
		do
			...
			do
				if [ -f M$3N$i-$j.dat ]
				then
					echo "plot 'sz/M$3N$i-$j.dat' using (\$1/$i):2 smooth unique title 'N=$i, M=$3' with lines;" >> ../gnuplot.gp
				fi
			done
		done
		;;
	...
	error)
		...
		for (( i=$1; i<=$2-1; i=i+2 ))
		do
			echo -n " 'error/M$3N$i-1.dat' using (\$1/$i):2 smooth unique title 'N=$i, M=$3' with lines," >> ../gnuplot.gp
		done
		i=$2
		echo -n " 'error/M$3N$i-1.dat' using (\$1/$i):2 smooth unique title 'N=$i, M=$3' with lines;" >> ../gnuplot.gp
		;;
	...
	vote)
		...
		for (( i=$1; i<=$2-1; i=i+3 ))
		do
			echo -n " 'vote/M$3N$i-1.dat' using (\$1/$i):2 smooth unique title 'N=$i, M=$3' with lines," >> ../gnuplot.gp
		done
		i=$2
		echo -n " 'vote/M$3N$i-1.dat' using (\$1/$i):2 smooth unique title 'N=$i, M=$3' with lines;" >> ../gnuplot.gp
		;;
